...
export async function GET(req: Request) {
  ...
  if (filteredVideos.length === 0) {
    return NextResponse.json({
      ...
      backupPhone: (facultyUser as any)?.backupPhone || "",
      cohort: facultyUser?.cohort || "June EduSkill",
      age: facultyUser?.age,
      dob: facultyUser?.dob,
      gender: (facultyUser as any)?.gender,
      tshirtSize: (facultyUser as any)?.tshirtSize,
      teachingSubject: facultyUser?.teachingSubject,
      examTarget: facultyUser?.examTarget,
      addressLine1: (facultyUser as any)?.addressLine1,
      addressLine2: (facultyUser as any)?.addressLine2,
      city: (facultyUser as any)?.city,
      state: (facultyUser as any)?.state,
      pincode: (facultyUser as any)?.pincode,
      editPermissionStatus: (facultyUser as any)?.editPermissionStatus || "none",
